...
onMounted(() => {
  ...
  // Helper: get points from MotionPath
  function getPathPoints(selector) {
    const raw = MotionPathPlugin.getRawPath(selector)[0]
    const points = []
    for (let i = 0; i < raw.length / 2; i++) {
      points.push({ x: raw[i * 2], y: raw[i * 2 + 1] })
    }
    return points
  }

  const treePathPoints = getPathPoints('.treePath')
  const treeBottomPoints = getPathPoints('.treeBottomPath')
  const randomScale = gsap.utils.random(0.5, 3, 0.001, true)

  // Create 201 particle clones
  for (let i = 0; i < 201; i++) {
    const shapeEl = document.querySelector(shapes[i % shapes.length]).cloneNode(true)
    mainSVG.value.appendChild(shapeEl)
    shapeEl.setAttribute('fill', colors[i % colors.length])
    shapeEl.setAttribute('class', 'particle')
    gsap.set(shapeEl, { x: -100, y: -100, transformOrigin: '50% 50%' })
    particles.push(shapeEl)
  }
  ...
  function animateParticle() {
    ...
    const a = particles[d]
    gsap.set(a, {
      x: gsap.getProperty(pContainer.value, 'x'),
      y: gsap.getProperty(pContainer.value, 'y'),
      scale: randomScale(),
    })
    gsap.timeline().to(a, {
      ...
      onStart: flicker,
      onStartParams: [a],
      onRepeat: (el) => gsap.set(el, { scale: randomScale() }),
      onRepeatParams: [a],
    })
    d = (d + 1) % 201
  }

  // Sparkle initial position
  gsap.set(sparkle.value, { transformOrigin: '50% 50%', y: -100 })
  gsap.set('svg', { visibility: 'visible' })

  // Main timelines
  const mainTimeline = gsap.timeline()
  const particleTimeline = gsap.timeline({ onUpdate: animateParticle })

  particleTimeline.to(pContainer.value, {
    duration: 6,
  ...
  particleTimeline.vars.onComplete = () => {
...
<template>
  <div class="myContainer">
    ...
    c53.9,15.4,47.8,40.1,27.7,44.7c-20,4.6-63.2,4.6-27.7,32.4s98.6,21.6,61.6,60.1"
        ...
        <g class="whole">
          <g ref="pContainer" class="pContainer"></g>
          ...
          <circle ref="sparkle" class="sparkle" fill="url(#dotGrad)" cx="0" cy="0" r="50" />
        </g>
        <foreignObject id="endMessage" x="0" y="550" width="800" height="250">
          <span class="endMessage"> Happ New Year {{ formatName(props.userName) }}! </span>
        </foreignObject>
        <!-- <div id="endMessage">Happ New Year!</div> -->
      </svg>
    </div>
  </div>
</template>
